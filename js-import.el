...
in a buffer local variable `js-import-cached-imports-in-buffer'.
  ...
  (with-current-buffer (or buffer helm-current-buffer)
    (let ((tick (buffer-modified-tick)))
      (if (and (eq js-import-cached-imports-in-buffer-tick tick)
               (not js-import-debug))
                      ...
                      ((looking-at-p "[\"']")
                       ...
                       (when-let* ((re-import-path
                                   ...
                                   (extracted-symbols (js-import-extract-imports
                                                       external-path)))
                         (setq extracted-symbols
                               (mapcar (lambda(it)
                                         (js-import-propertize
                                          it 'real-path external-path
                                          'display-path display-path))
                                       extracted-symbols))
        ...
        (let (p1 p2 item items real-name full-name)
          (while (js-import-re-search-forward js-import-regexp-name-set nil t 1)
            ...
            (setq item (js-import-make-index-item
                        ...
                        :local-name (or (nth 2 (split-string full-name))
                                        real-name)
                        :real-name real-name
                        :display-path display-path
                        :real-path real-path
...
(defun js-import-filter-with-prop(property value items)
  ...
  (seq-filter (lambda(str) (equal (js-import-get-prop str property) value))
              items))
...
(defun js-import-extract-all-exports(&optional path)
  "Return exports in PATH defined with CommonJs syntax."
  (let ((result)
        (symbols)
        (external-paths)
        (processed-paths))
    (push (js-import-path-to-real path) external-paths)
    ...
    (unless (js-import-inside-string-p)
      ...
      (cond ((looking-at-p "*[ \s\t\n]+as[ \s\t\n]")
            ...
            ((looking-at-p js-import-regexp-name-set)
             ...
             (setq exports (append exports (js-import-extract-var-exports))))
            ((looking-at-p "*[ \s\t]from")
...
(defun js-import-extract-esm-braced-exports(&optional real-path display-path)
  "Extracts exports beetween brackets."
  (save-excursion
    (when-let* ((brace-start (when (looking-at-p "{") (1+ (point))))
                (brace-end (save-excursion (forward-list)
                                           (1- (point))))
                (type 4))
      (save-restriction
        (narrow-to-region brace-start brace-end)
...
(defun js-import-extract-cjs-named-export(real-path &optional display-path)
  ...
  (let* ((real-name (js-import-which-word))
         (local-name (save-excursion
                       (js-import-re-search-forward (concat real-name "[\s\t\n]*") nil t 1)
                       (when (looking-at-p "=[^=]")
                         (forward-char)
                         (skip-chars-forward "\s\t\n")
...
(defun js-import-get-path-at-point()
  (interactive)
  (save-excursion
    (when (or (js-import-looking-at "from")
              (looking-at "[\s\t\n]*[\"']"))
      (re-search-forward "['\"]" nil t 1))
        ...
        (dotimes (idx (length open-parens))
          (let ((paren-pos (nth idx open-parens))
            ...
            (when parent (when (looking-back "=>" 1)
                           (backward-char 2)
                           (skip-chars-backward "\s\t\n")
                           (when-let ((child (js-import-maybe-make-child-at-point
                                              parent)))
                             (push child items)))
                                      ...
                                      ((looking-at js-import-regexp-name-set)
                                       ...
                                       (while (and (or (looking-at ",")
                                         ...
                                         (when-let ((child (js-import-maybe-make-child-at-point
                                           ...
                                           (skip-chars-backward "\s\t\n"))
                                         (unless
                                             (js-import-re-search-backward "," start t 1)
                                           (goto-char start)))))))
...
(defun js-import-previous-declaration(&optional pos)
  ...
  (while (and (save-excursion (js-import-re-search-backward js-import-delcaration-keywords--re nil t 1))
    ...
    (save-excursion
      ...
      (when-let* ((word (js-import-which-word))
        ...
        (if (js-import-valid-identifier-p word)
            (js-import-make-index-item word
                                       :pos pos
                                       :var-type token
                                       :real-name word
                                       :local-name word)
...
(defun js-import-jump-to-item-persistent(item)
  ...
  (let ((js-buffer (get-buffer-create "*helm-js-import*")))
    (cl-flet ((preview (item)
                       (setq item (js-import-try-find-definition item))
    ...
    (when export-item
      (if (js-import-get-prop export-item 'var-type)
        ...
        (let* ((path (or (js-import-get-prop export-item 'external-path)
               ...
               (name (or (js-import-get-prop export-item 'local-name)
                         (js-import-get-prop export-item 'external-name)
...
(defun js-import-find-in-exports(item path &optional default)
  (let* ((exports (js-import-extract-all-exports path))
...
(defun js-import-re-search-backward-inner (regexp &optional bound count)
  "Auxiliary function for `js-import-re-search-backward'."
  (let ((parse))
    (while (> count 0)
      (with-syntax-table js-import-mode-syntax-table
