...
(defvar js-imports-node-starts-re
  (concat "\\_<" (regexp-opt (delete "type" js-imports-node-starts-keywords) t) "\\_>"))
        ...
        ((looking-at-p "[.]")
         ...
         (when-let ((as-name (js-imports-which-word)))
           (js-imports-make-item
            ...
            :real-path path
            :pos (point))))))
(defun js-imports-get-obj-key (&optional re)
  ...
  (let ((start)
    ...
    (if (looking-at "[\\|['\"]")
      ...
      (when (looking-at (concat "[" re "]"))
        (setq start (point))
        (when (> (skip-chars-forward re) 0)
          (setq end (point)))))
    (when result (and start end (js-imports-make-item
                                 (buffer-substring-no-properties start end)
                                 :beg start
                                 :end end)))
    (setq result (and start end (buffer-substring-no-properties start end)))))
...
(defun js-imports-parse-object-from-string (&optional str)
  (js-imports-with-temp-buffer
   (insert str)
   (goto-char (point-min))
   (js-imports-parse-object)))

(defun js-imports-read-objects-display-fn (it)
  (string-join (split-string it) "\s"))
(defun js-imports-read-objects ()
  (interactive)
  (ivy-set-display-transformer 'js-imports-read-objects 'js-imports-read-objects-display-fn)
  (ivy-read "" (js-imports-parse-object-from-string (buffer-substring-no-properties (point-min) (point-max)))
            :action 'js-imports-jump-to-item-in-buffer
            :caller 'js-imports-read-objects))

(defun js-imports-parse-object-deep ()
  (when-let ((beg (when (looking-at-p "{")
                    (point))))
    (message "___________
            |	Beg 	|	%s"
             beg)
    (save-excursion
      (forward-sexp 1)
      (let ((body (buffer-substring-no-properties beg (point))))
        ...
        (js-imports-with-temp-buffer
         (insert body)
         (goto-char (point-min))
         (let ((alist (js-imports-parse-object))
               (pair)
               (result))
           (while (setq pair (pop alist))
             (let ((key (car pair))
                   ...
                   (first-char)
                   (last-char))
               (setq first-char (substring-no-properties value 0 1))
               (setq len (length value))
               (when (and (> len 1)
                          (string= first-char "{")
                          (string= (substring-no-properties value (1- len)) "}"))
                 (setq value (mapcar (lambda (it) (cons (js-imports-make-item (car it) :parent key)
                                                   (cdr it)))
                                     (js-imports-parse-object-from-string value))))
               (push (cons key value) result)))
           result))))))

(defun js-imports-skip-function ()
  ...
  (when (re-search-forward "{" nil t 1)
    ...
    (js-imports-skip-whitespace-forward))
  (point))
(defun js-imports-skip-arguments ()
  (forward-list 1)
  (js-imports-skip-whitespace-forward)
  (skip-chars-forward ":*_~$A-Za-z0-9")
  (js-imports-skip-whitespace-forward)
  ...
  (let ((open-chars-re "^\"'({[;}[]")
    ...
    (while (and
            ...
            (progn
              ...
              (save-excursion
                (setq b  (js-imports-re-search-forward "[^]\"'({[;}]+" nil t 1)))
              (cond ((and b a
                          (> a b))
                     (goto-char b))
                    ((and a)
    ...
    (and (looking-at stop-chars)
         (point))))
(defun js-imports-parse-object-recoursive ()
  (when (looking-at-p "{")
    (let ((start (point))
          (prev-p)
          (alist)
      ...
      (forward-char 1)
      (js-imports-skip-whitespace-forward)
      (while (setq key (js-imports-get-obj-key))
        (let ((value))
          (js-imports-skip-whitespace-forward)
          (setq divider (buffer-substring-no-properties (point) (+ 1 (point))))
          (when (member divider '("," ":"))
            (forward-char 1)
            (js-imports-skip-whitespace-forward))
          (pcase divider
            ...
            (":"
             ...
             (cond
              ((looking-at "(")
               (setq value-end (js-imports-skip-arguments)))
              ((looking-at "function[\\(\\*\s\t\n]")
               (setq value-end (js-imports-skip-function)))
              ((looking-at "{")
               (setq value (js-imports-parse-object-recoursive))
               (when (looking-at "}")
                 (forward-char 1)
                 (js-imports-skip-whitespace-forward))
               (js-imports-skip-whitespace-forward))
              ((looking-at "[[]")
               (setq value (js-imports-parse-array
                            'js-imports-parse-object-recoursive)))
              (t (js-imports-skip-to-char-same-scope ",")))
             (setq value-end (point))))
          (js-imports-skip-whitespace-forward)
          (when (looking-at ",")
            (forward-char 1)
...
(defun js-imports-parse-array (&optional parse-object-fn)
  (unless parse-object-fn (setq parse-object-fn (lambda ()
                                                  (let ((a (point)))
                                                    (forward-list 1)
  ...
  (when (looking-at "[[]")
    (let ((arr-start (point))
      ...
      (save-excursion
        ...
        (forward-char 1)
        (js-imports-skip-whitespace-forward)
        (setq arr-item-start (point))
        (while (and (< (point) arr-end) (js-imports-re-search-forward "[],{([]" arr-end t 1))
          (let ((prev-char (buffer-substring-no-properties (1- (point)) (point))))
            (message "______________________
                    ...
                    |	Arr Item Start 	|	%s
                    |	Arr Item End   	|	%s"
                     count prev-char arr-value arr-item-start arr-item-end)
            (cond ((and prev-char (string= prev-char "{")
                        (save-excursion
                          (forward-char -1)
                          (js-imports-skip-whitespace-backward)
                          (looking-back "[,[]" 0)))
                   (forward-char -1)
                   (setq arr-item-start nil)
                   (setq arr-item-end nil)
                   (setq arr-value (funcall parse-object-fn))
                   (push (cons count arr-value) arr-items)
                   (js-imports-skip-whitespace-forward)
                   (when (looking-at ",")
                     (forward-char 1)
                     (setq arr-item-start (point))))
                  ((and prev-char (string= prev-char "[")
                        (save-excursion
                          (forward-char -1)
                          (js-imports-skip-whitespace-backward)
                          (looking-back "[,[]" 0)))
                   ...
                   (setq arr-item-start nil)
                   (setq arr-item-end nil)
                   (setq arr-value (js-imports-parse-array parse-object-fn))
                   (push (cons count arr-value) arr-items)
                   (js-imports-skip-whitespace-forward)
                   (when (looking-at ",")
                     (forward-char 1)
                  ...
                  ((member prev-char '("[" "{" "("))
                   (forward-char -1)
                   (forward-list 1)
                   (setq arr-item-end (point)))
                  ((and prev-char (member prev-char '("]" ",")))
                   (setq arr-item-end (1- (point)))
                   (when (and arr-item-start arr-item-end)
                     (setq count (or (and count (1+ count))
                                     0))
                     (let ((value (js-imports-normalize-value (buffer-substring-no-properties
                                                               arr-item-start
                                                               arr-item-end))))
                       (push (cons count value)
                             arr-items)))
                   (js-imports-skip-whitespace-forward)
                   (if (looking-at "{")
                       (setq arr-item-start nil)
                     (setq arr-item-start (point))))))))
      ;; (setq arr-items (seq-map-indexed (lambda (it idx) (cons idx (cdr it)))
      ;;                                  (reverse arr-items)))
      (apply 'vector (mapcar 'cdr (reverse arr-items))))))

(defvar js-imports-current-obj nil)
(defun js-imports-parse-object ()
  (save-excursion
    (let ((vars (mapcar 'car (js-imports-skip-reserved-words))))
      (when-let ((id (car vars))
                 (var (cadr vars))
                 (var-type (member var '("const" "let" "var"))))
        (message "___________
                |	Id  	|	%s
                |	Var 	|	%s"
                 id var)
        ...
        (when (= 1 (skip-chars-forward "="))
          (js-imports-skip-whitespace-forward)
          (when (and (looking-at "[-*_~$A-Za-z0-9:.]+")
                     (not (js-imports-looking-at-function-expression)))
            (progn (skip-chars-forward "-*_~$A-Za-z0-9:.\s\t\n")
                   (when (looking-at "(")
                     (forward-char 1))))
          (when-let ((obj (save-excursion (and (looking-at "{")
                                               (js-imports-parse-object-recoursive)))))
            (with-current-buffer (get-buffer-create "*km-buffer*")
              (erase-buffer)
              (with-output-to-temp-buffer "*km-buffer*" (print obj))
              (pop-to-buffer))))))))
...
(defun js-imports-parse-object-keys ()
  ...
  (when-let ((start (when (looking-at-p "{")
    ...
    (let (children)
      ...
      (when children
        (reverse (seq-uniq (cl-remove 'null children)))))))
  ...
  (with-current-buffer js-imports-current-buffer
    (save-excursion
      (when-let* ((bounds (js-imports-inside-import-p (point)))
        ...
        (insert choice)))))
(defun js-imports-re-search-forward-same-scope-command ()
  (interactive)
  (js-imports-re-search-forward-in-scope js-imports-node-starts-re nil t 1))
(defun js-imports-get-nodes-in-scope ()
  (let ((nodes)
        (start (and (looking-at js-imports-node-starts-re)
                    (point)))
        (end)
        (word))
    (while (and
            (looking-at js-imports-node-starts-re nil t 2)
            (js-imports-re-search-forward-in-scope js-imports-node-starts-re 2)
            (setq start (or start end))
            (setq end (- (point) (length (js-imports-which-word)))))
      (js-imports-skip-whitespace-forward))))
(defun js-imports-re-search-forward-in-scope (regexp &rest bound no-error count)
  (let ((re-start)
        (init-pos)
        (open-chars-re (concat "\\" (string-join
                                     '("[" "(" "{")
                                     "\\|")))
        (scope-re (concat "\\" (string-join
                                '("[" "(" "{")
                                "\\|")))
        (scope-end-re "\\]\\|}\\|)")
        (scope-start)
        ...
        (found))
    (js-imports-skip-whitespace-forward)
    (when (looking-at open-chars-re)
      (forward-list 1))
    (while (and (setq re-start (save-excursion
                                 (js-imports-re-search-forward regexp nil t 1)))
                (null found)
                (null scope-end))
      (setq scope-start (save-excursion
                          (js-imports-re-search-forward scope-re nil t 1)))
      (message "___________________
              |	Scope Start 	|	%s"
               scope-start)
      ;; (cond (((and scope-start re-start)
      ;;         (> scope-start re-start))
      ;;        (setq found re-start))
      ;;       ((and scope-start)
      ;;        (forward-char -1)
      ;;        (if (looking-at open-chars-re)
      ;;            (forward-list 1))))
      (if (or (null scope-start)
              (> scope-start re-start))
          (setq found re-start)
