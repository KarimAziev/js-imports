    ...
    (if (looking-at "[\\|['\"]")
      ...
      (when (looking-at (concat "[" re "]"))
        ...
        (when (> (skip-chars-forward re) 0)
          (setq end (point)))))
    (and start end (js-imports-make-item
...
(defun js-imports-read-objects ()
  (interactive)
  (when-let ((obj (save-excursion (js-imports-parse-object))))
    (ivy-set-display-transformer 'js-imports-read-objects
                                 'js-imports-read-objects-display-fn)
    (ivy-read "" obj
              :action (lambda (it)
                        (print it)
                        (with-ivy-window
                          (let ((buff (current-buffer)))
                            (js-imports-jump-to-item-in-buffer (car it) buff))))
...
(defun js-imports-skip-arguments ()
  ...
  (when (looking-at-p "=>")
    ...
    (if (looking-at "[{(]")
      ...
      (js-imports-re-search-forward "[,;)}]]" nil t 1)))
  (point))
(defvar js-imports-matching-brackets
  '(("(" . ")")
    ("{" . "}")
    ("[" . "]"))
  "Alist with matching pairs of brackets.")

(defvar js-imports-open-brackets-regexp
  (concat "[" (string-join (mapcar 'car js-imports-matching-brackets)) "]")
  "Regexp matching opened brackets.")

(defvar js-imports-closed-brackets-regexp
  (concat "[" (string-join (mapcar 'cdr js-imports-matching-brackets)) "]")
  "Regexp matching opened brackets.")
...
(defun js-imports-skip-to-char-same-scope (&optional stop-chars)
  ...
  (let ((open-chars-re "[^]({[;}]+")
    ...
    (while (and
            ...
            (not (looking-at "[]})]"))
            (progn
              (setq search-point (save-excursion
                                   (js-imports-re-search-forward stop-chars nil t 1)))
              (save-excursion
                (setq scope-point  (js-imports-re-search-forward open-chars-re nil t 1)))
              (cond ((and scope-point search-point
                          (> search-point scope-point))
                     (goto-char scope-point))
                    ((and search-point)
                     (goto-char (1- search-point))
                     (setq stop t)))))
      (if (looking-at "[[({[]")
          (forward-list 1)
        (setq stop t)))
...
(defun js-imports-parse-object-recoursive (&optional parent-key)
  (when (looking-at-p "{")
    (let ((alist)
          (key)
          (divider)
      ...
      (while (setq key (js-imports-get-obj-key))
        (setq full-name (cond ((or (null parent-key)
        ...
        (setq key (js-imports-make-item key :id full-name))
        (let ((value)
              (value-start)
              (value-end))
          (js-imports-skip-whitespace-forward)
          ...
          (pcase divider
            ...
            (":"
             ...
             (cond
              ...
              ((looking-at "{")
               (setq value (js-imports-parse-object-recoursive full-name))
               (when (looking-at "}")
                 (forward-char 1)
                 (js-imports-skip-whitespace-forward))
               (js-imports-skip-whitespace-forward))
  ...
  (when (looking-at "[[]")
    (let ((arr-start (point))
      ...
      (save-excursion
        ...
        (if (looking-at "]")
            (setq arr-items (vector))
          (while (and (< (point) arr-end)
                      (js-imports-re-search-forward "[],{([]" arr-end t 1))
            (let ((prev-char (buffer-substring-no-properties
                              (1- (point)) (point))))
              (cond ((and prev-char (string= prev-char "{")
                     ...
                     (setq arr-value (js-imports-parse-object-recoursive))
                    ...
                    ((and prev-char (string= prev-char "[")
                     ...
                     (setq arr-item-start nil)
                     (setq arr-item-end nil)
                     (setq arr-value (js-imports-parse-array))
                     (push (cons count arr-value) arr-items)
                     (js-imports-skip-whitespace-forward)
                     (when (looking-at ",")
                       (forward-char 1)
                    ...
                    ((and prev-char (member prev-char '("]" ",")))
                     ...
                     (when (and arr-item-start arr-item-end)
                       (let ((value (js-imports-normalize-value
                                     (buffer-substring-no-properties
                                      arr-item-start
                                      arr-item-end))))
...
(defun js-imports-parse-object-from-string (&optional content)
  (js-imports-with-temp-buffer
   ...
   (cond ((looking-at "{")
         ...
         ((looking-at "\\[")
          (js-imports-parse-array)))))

(defun js-imports-get-object-bounds-at-point ()
  (when (looking-at "[{[]")
